...
let styles = {
  ...
  StyleSheet.create({
    ...
    "sectionDescription": textStyle(~marginTop=8.->dp, ~fontSize=18., ()),
    "highlight": textStyle(()),
  })
}
    ...
    <View style={styles["sectionContainer"]}>
      <Text
        style={
          ...
          array([
            styles["sectionTitle"],
            textStyle(~color=Belt.Option.getWithDefault(isDarkMode, false)  ? colors.white : colors.black, ()),
          ])
        }>
        {title->React.string}
      </Text>
...
let app = () => {
  let isDarkMode = useIsDarkMode()
  <SafeAreaView>
    <StatusBar barStyle={Belt.Option.getWithDefault(isDarkMode, false) ? #"light-content" : #"dark-content"} />
    <ScrollView
      contentInsetAdjustmentBehavior=#automatic
      style={
        open Style
        viewStyle(~backgroundColor=Belt.Option.getWithDefault(isDarkMode, false) ? colors.darker : colors.lighter, ())
      }>
      <Header />
      <Section title={"Step One ( This is rescript file)"}>
        {"Edit "->React.string}
